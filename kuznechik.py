...
def main():
    with open("keys/A_B.key", "rb") as f:
        key = f.read(32)

    k = Kuznechik(key)

    msg_restored = int.from_bytes(key, byteorder='big')

    print("Restored message as int:", msg_restored)
    print("Hex representation:", ''.join(f"{b:02x}" for b in key))


    m = 16

    with open("texts/my_text.txt", 'r') as f:
        data = f.read()
    f.close()
    ...
    encrypt = b''.join([bytes(k.encrypt(blk.encode())) for blk in blks])

    with open("texts/cipher_text.bin", 'wb') as f:
        f.write(encrypt)
    ...
    with open("texts/test_Khabalov", 'rb') as f:
        encrypt = f.read()
    f.close()
    n = len(encrypt)
    blks = [encrypt[i:i+m] for i in range(0, n-n%m, m)]
    decrypt = ''.join([''.join([chr(x) for x in k.decrypt(blk)]) for blk in blks])
    print(decrypt.encode())
    
    with open("texts/decrypted.txt", 'w') as f:
        f.write(decrypt)
    f.close()
